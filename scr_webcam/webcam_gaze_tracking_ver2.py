...
class GazeEstimator:
    ...
    #   [6,7]  head_yaw/pitch          — conditioning: lets model learn pose correction from data
...
class GazeCalibrator:
    def __init__(self, outlier_mad_k=2.5, eval_models: bool = False):
        ...
        self.model            = None   # single deployed model pipeline
        self._out_corrector   = None   # LinearRegression post-correction
    ...
    def _reject_outliers(self, X, y):
        n_feat = X.shape[1]
        if n_feat >= 522:
            l2cs_slice = slice(516, 518)   # full: L2CS at 516-518
        else:
            l2cs_slice = slice(0, 2)       # slim: l2cs_yaw/pitch at [0,1]
    ...
    def _evaluate_models(self, X_train, y_train, X_test, y_test):
        ...
        candidates = {
            ...
            "Ridge+Poly2(10)": make_pipeline(StandardScaler(),
                                             ...
                                             Ridge(alpha=10.0)),
            "Ridge+Poly2(50)": make_pipeline(StandardScaler(),
                                             PolynomialFeatures(degree=2, include_bias=False),
                                             Ridge(alpha=50.0)),
            "MLP":             make_pipeline(
                                   StandardScaler(),
    ...
    def fit(self, features, targets_px):
        ...
        if self.eval_models:
            ...
            self.model           = pipeline
            self.best_model_name = "Ridge+Poly2(α=10)"
            print(f"\n[Calibration] Ridge+Poly2(α=10) fitted  dim={n_feat}  n={len(X_clean)}")
    ...
    def predict(self, feature):
        if not self.is_calibrated:
            return None
        raw = self.model.predict(np.array(feature, dtype=np.float64).reshape(1, -1))
