...
[Performance]
  - Slim 10-dim features by default (vs 522-dim); --full-features to restore
  - Ridge+Poly2 by default; --eval-models benchmarks Ridge/Ridge+Poly2/Ridge+Poly3/MLP
    at calibration time and deploys the winner only
  - Japanese text rendered as cached BGRA sprites — no full-frame PIL
    round-trips per call
  - AsyncCamera thread yields CPU via 1 ms sleep per loop

[Stability]
  - --save-calib FILE / --load-calib FILE: persist trained model (joblib)
  - /gaze/lost OSC sent when face tracking drops; Kalman smoother is reset
  - --blink-ratio FLOAT (default 0.75): tune blink detection for your eyes
  - EAR adaptive threshold requires >= 30 history frames before activating (was 15)
  - BBoxStabilizer alpha raised to 0.40 (was 0.20) — face crop tracks faster
"""
import warnings
...
def _render_text_sprite(text: str, font_size: int, color_bgr: tuple) -> np.ndarray:
    """Render text once into a minimal BGRA sprite, cached by content."""
    font = _get_ja_font(font_size)
    fill_rgba = (color_bgr[2], color_bgr[1], color_bgr[0], 255)
    ...
    try:
        ...
        w, h, ox, oy = max(1, tw + 2), max(1, th + 2), 1, 1
    sprite_pil = Image.new("RGBA", (w, h), (0, 0, 0, 0))
    ImageDraw.Draw(sprite_pil).text((ox, oy), text, font=font, fill=fill_rgba)
    arr = np.array(sprite_pil, dtype=np.uint8)   # H×W×4 RGBA
    return arr[:, :, [2, 1, 0, 3]]               # → BGRA

def put_text_ja(img: np.ndarray, text: str, org: tuple,
                font_size: int = 24, color_bgr=(255, 255, 255)) -> None:
    """Alpha-blit a cached text sprite onto a BGR image in-place."""
    if not text:
        return
    sprite = _render_text_sprite(text, font_size, tuple(color_bgr))
    ...
    patch = sprite[sy0:sy0 + (dy1 - dy0), sx0:sx0 + (dx1 - dx0)]
    alpha = patch[:, :, 3:4].astype(np.float32) / 255.0
    img[dy0:dy1, dx0:dx1] = (roi * (1.0 - alpha)
...
# Landmark Indices
# ──────────────────────────────────────────────────────────────────────
# This project uses mediapipe Face Mesh
...
FACE_PLANE_IDX = np.array([
    10, 109, 338,          # forehead (left, centre, right)
    234, 454,              # cheek outer (left, right)
    93,  323,              # cheek inner
    152, 176, 400,         # chin and jaw corners
    168, 6,                # nose bridge
    21,  251,              # temple
], dtype=np.int32)
...
class KalmanEMAFilter:
    ...
    def reset(self):
        """Fix 7: no-arg reset — clears state; update() re-initializes on next call."""
        self.ema  = None
        self.init = False
...
class BBoxStabilizer:
    def __init__(self, alpha=0.40):   # Fix 3: raised from 0.20 to 0.40
        self.alpha = alpha
        self.bbox  = None
...
class GazeEstimator:
    ...
    def __init__(self, img_w, img_h, model_path="models/L2CSNet_gaze360.pkl",
        ...
        self._l2cs_frame   = 0
        self._l2cs_yaw_cache   = 0.0
        self._l2cs_pitch_cache = 0.0
        self.feat_dim = self.SLIM_FEAT_DIM if slim_features else self.FULL_FEAT_DIM
    ...
    def process(self, frame):
        ...
        # Fix 4: raise minimum EAR history from 15 to 30
        thr = (float(np.mean(self._ear_history)) * self.blink_ratio
        ...
        # Vt rows: [max-variance (≈left-right), mid-variance (≈up-down), min-variance (≈normal)]
        x_axis = Vt[0]; y_axis = Vt[1]; z_axis = Vt[2]
        # Canonical orientation: x points right (+x image), z points toward camera (+z)
        ...
        # L2CS — run every l2cs_every frames; use cached values on skipped frames
        self._l2cs_frame += 1
        l2cs_yaw_local   = self._l2cs_yaw_cache
        l2cs_pitch_local = self._l2cs_pitch_cache
        if self._l2cs_frame % self.l2cs_every == 0:
            ...
            if crop.size > 0:
                ...
                with torch.inference_mode():
                    ...
                    # EMA smoothing on L2CS output — absorbs single-frame spikes
                    # from non-frontal face crops (α=0.4: ~3-frame effective window)
                    α = 0.4
                    self._l2cs_yaw_cache   = α * l2cs_yaw_local   + (1-α) * self._l2cs_yaw_cache
                    self._l2cs_pitch_cache = α * l2cs_pitch_local + (1-α) * self._l2cs_pitch_cache
                    l2cs_yaw_local   = self._l2cs_yaw_cache
                    l2cs_pitch_local = self._l2cs_pitch_cache
        ...
        # Iris ring features — 5 landmarks per eye in head-local normalized coordinates
        # Gives ellipse shape (center + orientation + axis ratio) = pure geometric gaze signal
        ...
        iris_l_ring = []
        for idx in L_IRIS_RING:
            v = R_head.T @ (all_points[idx] - l_eye_center)
            iris_l_ring.extend([v[0] / l_eye_width, v[1] / l_eye_width])

        iris_r_ring = []
        for idx in R_IRIS_RING:
            v = R_head.T @ (all_points[idx] - r_eye_center)
            iris_r_ring.extend([v[0] / r_eye_width, v[1] / r_eye_width])
        ...
        if self.slim_features:
            ...
            face_norm_feats, _ = self._face_normalized_features(lm, all_points_px)
            features = np.concatenate([
                face_norm_feats,
                [head_x, head_y, head_size],
                [l2cs_yaw_local, l2cs_pitch_local],
                iris_l_ring[:2], iris_r_ring[:2],  # center point (landmark 468/473)
            ]).astype(np.float64)
...
# ──────────────────────────────────────────────────────────────────────
# GazeCalibrator (Ridge+Poly2 default; optional multi-model eval)
# ──────────────────────────────────────────────────────────────────────
class GazeCalibrator:
    def __init__(self, outlier_mad_k=2.5, eval_models: bool = False):
        self._outlier_k       = outlier_mad_k
        self.eval_models      = eval_models
        self.model            = None
        self._out_corrector   = None
    ...
    def _reject_outliers(self, X, y, phases=None):
        n_feat = X.shape[1]
        if n_feat >= 522:
            l2cs_slice = slice(516, 518)
        else:
            l2cs_slice = slice(0, 2)

        X_out, y_out, dropped = [], [], 0
        k = self._outlier_k

        # Multi-pose calibration: the same target has very different l2cs values
        # across phases (head turned left → l2cs_yaw shifts ~20°). Grouping all
        # phases together treats valid phase-data as outliers (~23% false rejection).
        # When phase labels are available, reject within (target, phase) pairs only.
        unique_targets = np.unique(y, axis=0)
        for tgt in unique_targets:
            mask_tgt = np.all(y == tgt, axis=1)
            if phases is not None:
                phase_groups = np.unique(phases[mask_tgt])
            else:
                phase_groups = [None]

            for ph in phase_groups:
                if ph is not None:
                    mask = mask_tgt & (phases == ph)
                else:
                    mask = mask_tgt
                Xi, yi = X[mask], y[mask]
                if len(Xi) <= 4:
    ...
    def _evaluate_models(self, X_train, y_train, X_test, y_test):
        ...
        candidates = {
            ...
            "Ridge+Poly2(1)":  make_pipeline(StandardScaler(),
                                             PolynomialFeatures(degree=2, include_bias=False),
                                             Ridge(alpha=1.0)),
            "Ridge+Poly2(10)": make_pipeline(StandardScaler(),
                                             PolynomialFeatures(degree=2, include_bias=False),
                                             Ridge(alpha=10.0)),
            "MLP":             make_pipeline(
                                   StandardScaler(),
        ...
        for name, pipeline in candidates.items():
            ...
            err = float(np.mean(np.linalg.norm(pipeline.predict(X_test) - y_test, axis=1)))
            print(f"  {name:<18} : {err:5.1f} px  ({time.time()-t0:.2f}s)")
            results.append((name, err, pipeline))
    ...
    def fit(self, features, targets_px, phases=None):
        ...
        X_clean, y_clean, n_dropped = self._reject_outliers(X, y, phases_arr)
        print(f"\n[Data Prep] Outlier rejection: {n_dropped} removed "
              f"({len(X_clean)}/{len(X)} kept)")
        if len(X_clean) < 50:
            print("  WARNING: Few clean samples — using all data.")
            X_clean, y_clean = X, y
        ...
        if self.eval_models:
            ...
            results = self._evaluate_models(X_tr, y_tr, X_te, y_te)
            best_name, _, best_pipeline = results[0]
            print(f"\n  Winner: {best_name} — re-training on all clean data…")
            ...
            self.model           = pipeline
            self.best_model_name = "Ridge+Poly2(α=1)"
            print(f"\n[Calibration] Ridge+Poly2(α=1) fitted  dim={n_feat}  n={len(X_clean)}")

        pred_all = self.model.predict(X_clean)
        rmse = np.sqrt(np.mean((pred_all - y_clean) ** 2, axis=0))
        print(f"  Train RMSE  x={rmse[0]:.1f}px  y={rmse[1]:.1f}px  "
              f"diag={np.hypot(rmse[0], rmse[1]):.1f}px")

        # Corrector anchored on per-target mean predictions.
        # Fitting on all frames lets mid-range samples dominate and corners stay shrunk.
        # Using only the 9 calibration-point means forces the affine map to hit corners.
        unique_targets = np.unique(y_clean, axis=0)
        anchor_preds, anchor_targets = [], []
        for t in unique_targets:
            mask = np.all(y_clean == t, axis=1)
            anchor_preds.append(self.model.predict(X_clean[mask]).mean(axis=0))
            anchor_targets.append(t)
        self._out_corrector = LinearRegression().fit(
            np.array(anchor_preds), np.array(anchor_targets))
        corrected = self._out_corrector.predict(pred_all)
        rmse_c = np.sqrt(np.mean((corrected - y_clean) ** 2, axis=0))
        print(f"  Post-correction RMSE  x={rmse_c[0]:.1f}px  y={rmse_c[1]:.1f}px  "
              f"diag={np.hypot(rmse_c[0], rmse_c[1]):.1f}px")
    ...
    def predict(self, feature):
        if not self.is_calibrated:
            return None
        f = np.array(feature, dtype=np.float64)
        raw = self.model.predict(f.reshape(1, -1))
        if self._out_corrector is not None:
            return self._out_corrector.predict(raw)[0]
        return raw[0]
    ...
    def load(cls, path: str) -> "GazeCalibrator":
        ...
        obj._out_corrector  = data.get("out_corrector", None)
        obj.best_model_name = data["best_model_name"]
        obj._n_features     = data["n_features"]
        obj.is_calibrated   = True
        print(f"[Calibration] Loaded ← {path}  "
              f"(model: {obj.best_model_name}, dim: {obj._n_features})")
        return obj
...
def build_calibration_grid(W, H, rows=3, cols=3,
    ...
    def _positions(n, origin, span):
        if edge_focus and n > 2:
            # Chebyshev nodes of the 2nd kind: optimal polynomial interpolation nodes.
            # Concentrates samples at edges, minimising Runge oscillation in Ridge+Poly2.
            # t[k] = (1 - cos(k*pi/(n-1))) / 2  →  [0, dense-near-0, ..., dense-near-1, 1]
            t = (1 - np.cos(np.linspace(0, np.pi, n))) / 2
        else:
            t = np.linspace(0, 1, n)
        return [origin + int(v * span) for v in t]

    xs = _positions(cols, mx, gw)
    ys = _positions(rows, my, gh)

    pts = []
    for r_i, y_val in enumerate(ys):
        row = [(x_val, y_val) for x_val in xs]
        if order == "serpentine" and r_i % 2 == 1:
            row = row[::-1]
        pts.extend(row)
    return pts
...
def run_multi_pose_calibration(estimator, cam, calib, W, H, win_name,
    ...
    grid_mode = "Chebyshev (edge-dense)" if edge_focus else "uniform"
    print(f"\n=== Extended Multi-Pose Calibration ===")
    print(f"  Grid   : {rows}×{cols} = {len(pts)} points per pose  [{grid_mode}]")
    print(f"  Phases : {len(CALIB_PHASES)} ({', '.join(p['name'] for p in CALIB_PHASES)})")
    print(f"  Total  : {total_pts} points")
    print(f"  Feature: {estimator.feat_dim} dims")

    all_feats  = []
    all_targs  = []
    all_phases = []   # phase index per frame — used for per-phase outlier rejection
    ...
    for phase_idx, phase in enumerate(CALIB_PHASES):
        ...
        all_phases.extend([phase_idx] * len(feats))
    ...
    calib.fit(all_feats, all_targs, all_phases)
    return True
...
def main():
    ...
    ap.add_argument('--margin',       type=float, default=0.02,
                    help='Screen edge margin as fraction (default 0.02 = 2%%)')
    ap.add_argument('--edge-focus',   action='store_true',
                    help='Chebyshev node grid: dense at edges, sparse at centre')
    ...
    # New CLI arguments (Fix 8 / module-level additions)
    ap.add_argument('--save-calib',   type=str,   default=None, metavar='FILE',
                    help='Path to save calibration after fitting (e.g. calib_v2.pkl)')
    ap.add_argument('--load-calib',   type=str,   default=None, metavar='FILE',
                    help='Skip calibration and load from file')
    ap.add_argument('--blink-ratio',  type=float, default=0.75,
                    help='Blink detection ratio threshold (default 0.75)')
    ap.add_argument('--l2cs-every',  type=int,   default=1,
                    help='Run L2CS every N frames (default 1; 2-3 reduces CPU ~50%%)')
    ...
    # Fix 8: load calibration from file if --load-calib given
    if args.load_calib:
        calib = GazeCalibrator.load(args.load_calib)
        mode = "TRACKING"

    cv2.namedWindow(WIN, cv2.WINDOW_NORMAL)
    if args.calibrate and mode == "CALIBRATION":
    ...
    try:
        if mode == "CALIBRATION":
            ...
            # Fix 8: save calibration to file if --save-calib given
            if args.save_calib:
        ...
        while mode == "TRACKING":
            ...
            if feat is not None and not is_blink:
                ...
                face_lost_frames += 1
                if face_lost_frames == FACE_LOST_GRACE and tracking_face:
                    osc.send_message("/gaze/lost", 1.0)
                    tracking_face = False
                # Smoother NOT reset: freeze last position, recover smoothly
            ...
            if args.preview:
                ...
                py_ = int(last_y * display.shape[0] / H)
